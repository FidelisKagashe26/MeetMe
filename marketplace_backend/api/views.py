...
from .models import (
    SellerProfile, Location, Category, Product, ProductImage, Review, Favorite
)
from .serializers import (
    UserSerializer, UserRegistrationSerializer, SellerProfileSerializer,
    SellerProfileCreateSerializer, LocationSerializer, CategorySerializer,
    ProductSerializer, ProductCreateSerializer, ProductImageSerializer,
    ReviewSerializer, FavoriteSerializer, NearbySearchSerializer
)
from .utils import (
    MapboxService, calculate_distance_km, filter_by_radius,
    add_distance_to_queryset, sort_by_distance
)
...
class SellerProfileViewSet(viewsets.ModelViewSet):
    ...
    def nearby(self, request):
        """
        Get nearby sellers based on user's location
        """
        ...
        sellers = self.queryset.filter(location__isnull=False)
...
class ProductViewSet(viewsets.ModelViewSet):
    ...
    queryset = Product.objects.select_related(
        'seller', 'seller__location', 'category'
    ...
    def perform_create(self, serializer):
        try:
            ...
            raise Response({
                'error': 'You must create a seller profile first'
            }, status=status.HTTP_400_BAD_REQUEST)
    ...
    def search_nearby(self, request):
        """
        Search for products nearby with filters
        """
...
class ReviewViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        avg_rating = Review.objects.filter(seller=seller).aggregate(Avg('rating'))['rating__avg']
...
def reverse_geocode(request):
    ...
    mapbox_service = MapboxService()
    result = mapbox_service.reverse_geocode(float(lon), float(lat))
